...
interface CropperRootProps extends React.HTMLAttributes<HTMLDivElement> {
  image: string;
  initialCropArea: (Area & { scale: number }) | null;
  cropPadding?: number;
  aspectRatio?: number;
  minZoom?: number;
  maxZoom?: number;
...
const CropperRoot: React.FC<CropperRootProps> = ({
  ...
  const calculateTransformFromArea = (
    area: Area | null,
    zoom: number
    ...
    const offsetX = topLeftOffsetX - (cropAreaWidth - scaledWrapperWidth) / 2;

    const offsetY = topLeftOffsetY - (cropAreaHeight - scaledWrapperHeight) / 2;
  ...
  const handleReset = useCallback(() => {
    if (initialCropArea) {
      ...
      latestZoomRef.current = transform?.zoom ?? minZoom;
    }
